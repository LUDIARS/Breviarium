...
${workflowStrip(o.workflow)}${toolChips(o.tools)}${sprintChips(o.sprints)}</li>`;
}
...
export function renderIndexPage(portfolio: readonly ProjectOverview[], notice: PageNotice, level: AccessLevel): string {
  ...
  const list =
    portfolio.length === 0
      ...
      : `${workflowLegend()}<ul class="project-list">${portfolio.map(projectCard).join('')}</ul>`;
  const form = local
...
<form method="post" action="/projects">${projectFields('new')}<div class="actions"><button type="submit">登録</button></div></form></details>`
    : '';
  return page('Breviarium — プロジェクト総覧', `${banners(notice)}<h1>プロジェクト総覧</h1><p class="small muted">状態はスタートアップ / スプリント N 週目 / リリース済み / 運用中。スタートアップ 2 段・PDCA ループ 4 段・アナライズ 3 項目の順に並ぶ。チップはツールごとの最も低いクラス (— は未計測)。</p>${list}${form}`, level);
}

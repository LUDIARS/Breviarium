...
/**
 * Per-source snapshot state. With `withActions` (local operator) each row has its own refresh
 * button (form POST, no script); without it the operation column is left out entirely.
 */
export function sourceTable(code: string, sources: readonly SourceView[], withActions: boolean): string {
  ...
  const rows = sources
    .map((s) => {
      ...
      const button = withActions
        ? `\n<td><form class="inline" method="post" action="${esc(action)}"><input type="hidden" name="source" value="${esc(s.source)}"><button type="submit" class="secondary" aria-label="${esc(`${s.label} を更新`)}">更新</button></form></td>`
        : '';
...
<td class="small">${s.error ? esc(s.error) : '<span class="muted">—</span>'}</td>${button}</tr>`;
    })
    .join('');
  const operation = withActions ? '<th scope="col">操作</th>' : '';
  return `<div class="table-scroll"><table><thead><tr><th scope="col">ソース</th><th scope="col">鮮度</th><th scope="col">取得日時</th><th scope="col">最終試行</th><th scope="col">エラー</th>${operation}</tr></thead><tbody>${rows}</tbody></table></div>`;
}

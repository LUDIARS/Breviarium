...
/**
 * Daily: days since the latest Actio task update. Actio's project sprint aggregate (Actio spec §6.4) has
...
function daily(actio: ActioEvidence | null, current: CurrentSprint | null): SprintMetric {
  const base = { id: 'daily', title: 'デイリーの動き (直近のタスク更新からの経過日数)', value: null, unit: '日' } as const;
...
/**
 * Refinement: the estimated share of the unassigned backlog. Actio's aggregate counts the unassigned
...
function refinement(actio: ActioEvidence | null): SprintMetric {
  const base = { id: 'refinement', title: 'バックログ整理度 (未割付バックログの見積り済み割合)', value: null, unit: '%' } as const;
  if (!actio) return { ...base, reasons: ['Actio 未取得'] };
